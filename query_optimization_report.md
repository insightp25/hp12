...
* 테스트는 쿼리 분석후 애플리케이션에서 가장 슬로우할 것으로 예상되는 쿼리를 선정하였고, 해당 쿼리에 대한 인덱스 적용 전과 적용 후의 탐색 시간을 측정 및 비교하였습니다.
...
* 총 1,000,050 row로 구성
* 랜덤을 활용해 예약의 `status`값을 8:2(`FINALIZED`:`ABOLISHED`) 비율로 불규칙하게 골고루 삽입되도록 작성하였습니다.
```
// MySQL 콘솔 reservation 테이블 조회 샘플
